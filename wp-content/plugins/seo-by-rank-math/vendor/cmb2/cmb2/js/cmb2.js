...
(function (window, document, $, cmb, undefined) {
  ...
  cmb.resetGroupTitles = function ($row, newIterator, groupTitle) {
    if (groupTitle) {
      ...
      if ($rowTitle.length) {
        $rowTitle.text(
          groupTitle.replace("{#}", parseInt(newIterator, 10) + 1)
        );
      }
    }
  };
    ...
    $parent
      ...
      .each(function () {
        ...
        var prevNum = parseInt(
          $el.parents(".cmb-repeatable-grouping").data("iterator"),
          10
        );
  ...
  cmb.shiftRows = function (evt) {
    ...
    $from
      .attr("data-iterator", toIterator)
      .find(cmb.repeatEls)
      .each(function () {
        cmb.updateNameAttr($(this), fromIterator, toIterator);
      });
    $goto
      .attr("data-iterator", fromIterator)
      .find(cmb.repeatEls)
      .each(function () {
    ...
    var groupTitle = $this
      .parents(".cmb-repeatable-group")
      .find("[data-grouptitle]")
      .data("grouptitle");
    if (groupTitle) {
  ...
  cmb.makeRepeatableSortable = function ($row) {
    var $repeatables = ($row || cmb.metabox()).find(
      ".cmb-repeat-table .cmb-field-list"
    );
