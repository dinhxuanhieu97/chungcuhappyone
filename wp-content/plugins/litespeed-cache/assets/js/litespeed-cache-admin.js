...
(function ($) {
  ...
  jQuery(document).ready(function () {
    ...
    $("[data-litespeed-cfm]").on("click", function (event) {
      if (
        confirm($.trim($(this).data("litespeed-cfm")).replace(/\\n/g, "\n"))
      ) {
        return true;
      }
    ...
    if ($("[data-litespeed-tab]").length > 0) {
      // display default tab
      var litespeed_tab_current = document.cookie.replace(
        /(?:(?:^|.*;\s*)litespeed_tab\s*\=\s*([^;]*).*$)|^.*$/,
        "$1"
      );
      ...
      if (
        !litespeed_tab_current ||
        !$('[data-litespeed-tab="' + litespeed_tab_current + '"]').length
      ) {
        litespeed_tab_current = $("[data-litespeed-tab]")
          .first()
          .data("litespeed-tab");
      }
    ...
    if ($('input[name="LSCWP_CTRL"]').length > 0) {
      ...
      btn
        .clone()
        .addClass("litespeed-float-submit")
        .removeAttr("id")
        .insertAfter(btn);
    }
      ...
      $(".litespeed-toggle").on("click", function (e) {
        ...
        $.ajax({
          ...
          success: function (data) {
            $(that)
              .toggleClass(
                "litespeed-toggle-btn-default litespeed-toggleoff",
                data == 0
              )
          ...
          error: function (xhr, error) {
            ...
            console.log(error);
            console.log(
              "litespeed-crawler-ajax: option failed to save due to some error"
            );
          },
        });
      });
  ...
  jQuery.getJSON(_litespeed_crawler_url, function (meta) {
    ...
    if (meta && "list_size" in meta) {
      ...
      if (new_meta != _litespeed_meta) {
        ...
        if (log_length > 50) {
          jQuery(
            ".litespeed-shell-body li:lt(" + (log_length - 50) + ")"
          ).remove();
        }
        // scroll to end
    ...
    _litespeed_shell_handle = window.setTimeout(
      _litespeed_dynamic_timeout,
      _litespeed_shell_interval * 1000
    );
  });
}
...
function litespeed_display_interval_reset() {
  ...
  _litespeed_shell_display_handle = window.setInterval(
    _litespeed_display_interval,
    1000
  );
...
function _litespeed_display_interval() {
  ...
  jQuery(".litespeed-shell-header-bar")
    .stop()
    .animate(
      { width: litespeed_get_percent(num, _litespeed_shell_interval) + "%" },
      1000
    );
  if (num > 0) num--;
  if (num < 0) num = 0;
