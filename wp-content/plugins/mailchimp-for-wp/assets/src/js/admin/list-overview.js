...
function view(data) {
  return [
    ...
    m("table.widefat.striped", [
      m("thead", [m("tr", [m("th", "Name"), m("th", "Tag"), m("th", "Type")])]),
      m(
        "tbody",
        data.merge_fields.map((f) =>
          m("tr", [
            m("td", [f.name, f.required && m("span.mc4wp-red", "*")]),
            m("td", [m("code", f.tag)]),
            m("td", [
              f.type,
              " ",
              f.options && f.options.date_format
    ...
    data.interest_categories.length > 0 && [
      ...
      m("table.striped.widefat", [
        m("thead", [
          m("tr", [m("th", "Name"), m("th", "Type"), m("th", "Interests")]),
        ]),
        m(
          "tbody",
          data.interest_categories.map((f) =>
            m("tr", [
              m("td", [
                m("strong", f.title),
                m("br"),
