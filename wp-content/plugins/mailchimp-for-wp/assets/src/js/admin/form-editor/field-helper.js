...
function setActiveField(name) {
  ...
  if (
    ...
    fieldConfig.choices.length > 0
  ) {
    fieldConfig.value = fieldConfig.choices
      .map(function (c) {
        return c.label;
      })
      .join("|");
...
function view() {
  ...
  const fieldsChoice = m("div#mc4wp-available-fields.mc4wp-margin-s", [
    ...
    [i18n.listFields, i18n.interestCategories, i18n.formFields].map(function (
      category
    ) {
  ...
  if (fieldConfig) {
    form = m(
      Overlay,
      { onClose: () => setActiveField(null) }, // field wizard
