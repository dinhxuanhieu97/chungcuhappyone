...
(function (tinymce) {
  tinymce.PluginManager.add("wptadv", function (editor) {
    ...
    editor.on("init", function () {
      if (noAutop) {
        editor.on("SaveContent", function (event) {
          event.content = event.content.replace(
            /caption\](\s|<br[^>]*>|<p>&nbsp;<\/p>)*\[caption/g,
            "caption] [caption"
          );

          event.content = event.content.replace(
            /<(object|audio|video)[\s\S]+?<\/\1>/g,
            function (match) {
      ...
      try {
        if (
          editor.plugins.searchreplace &&
          !editor.controlManager.buttons.searchreplace
        ) {
    ...
    editor.addCommand("tmaRemoveTableStyles", function () {
      ...
      if (table) {
        editor
          .$(table)
          .attr(attr)
          .find("tr, th, td, thead, tbody, tfoot")
      ...
      if (editor.settings.classic_block_editor) {
        editor.on(
          ...
          function (event) {
            ...
            if (
              blocks.length === 1 &&
              blocks.attr("data-type") === "core/freeform"
            ) {
