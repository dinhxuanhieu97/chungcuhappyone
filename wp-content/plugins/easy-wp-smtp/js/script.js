...
jQuery(function ($) {
  ...
  $("a.nav-tab").click(function (e) {
    if ($(this).attr("data-tab-name") !== swpsmtp_activeTab) {
      $(
        'div.swpsmtp-tab-container[data-tab-name="' + swpsmtp_activeTab + '"]'
      ).hide();
      $('a.nav-tab[data-tab-name="' + swpsmtp_activeTab + '"]').removeClass(
        "nav-tab-active"
      );
...
jQuery(function ($) {
  ...
  $("#swpsmtp_enable_domain_check").change(function () {
    $('input[name="swpsmtp_allowed_domains"]').prop(
      "disabled",
      !$(this).is(":checked")
    );
    $('input[name="swpsmtp_block_all_emails"]').prop(
      "disabled",
      !$(this).is(":checked")
    );
  });
    ...
    if (confirm(easywpsmtp.str.clear_log)) {
      var req = jQuery.ajax({
        ...
        data: {
          ...
          nonce: easywpsmtp.clear_log_nonce,
        },
      });
